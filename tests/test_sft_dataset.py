...
class SFTDatasetTests(unittest.TestCase):
    ...
    def test_collator_pads_only_to_longest_example(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            tokenizer = train_test_tokenizer(root)
            jsonl_path = write_jsonl(
                ...
                [
                    ...
                    {
                        "system": "",
                        "user": "Explain this in somewhat more detail",
                        "assistant": "This response is intentionally longer than hello.",
                    },
                ],
            )
            ...
            self.assertTrue(
                all(token == -100 for token in label_batch[shorter_index, shorter_length:].tolist())
            )
